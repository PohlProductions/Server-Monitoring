...
(crontab -l | grep -v -F "/monitoring/monitoring.sh" ; echo "*/1 * * * * /monitoring/monitoring.sh $REMOTE_HOST $SMTPSERVER $FROM $TO $USER $PASSWORD >> /var/log/monitoring/monitoring.log") | crontab -
